...
var mobile = (function (exports, domGlobals) {
    ...
    var rawSet = function (dom, key, value$$1) {
      if (isString(value$$1) || isBoolean(value$$1) || isNumber(value$$1)) {
        ...
        domGlobals.console.error('Invalid call to Attr.set. Key ', key, ':: Value ', value$$1, ':: Element ', dom);
    ...
    var internalSet = function (dom, property, value$$1) {
      if (!isString(value$$1)) {
        domGlobals.console.error('Invalid call to CSS.set. Property ', property, ':: Value ', value$$1, ':: Element ', dom);
    ...
    var events$8 = derive([can(focus$1(), function (component, simulatedEvent) {
        ...
        if (isRecursive(component, originator, target)) {
          domGlobals.console.warn(focus$1() + ' did not get interpreted by the desired target. ' + '\nOriginator: ' + element(originator) + '\nTarget: ' + element(target) + '\nCheck the ' + focus$1() + ' event handlers');
          return false;
        } else {
          return true;
        }
    ...
    var getEnd = function (element) {
      return contains(elementsWithCursorPosition, name(element)) ? 1 : getOption(element).fold(function () {
    ...
    var getRectsFromRange = function (range$$1) {
      if (!range$$1.collapsed) {
        ...
        var start_1 = Element.fromDom(range$$1.startContainer);
    ...
    var setup$2 = function (container, rawSettings) {
      ...
      var simpleEvents = map$1(pointerEvents.concat([
        ...
        return bind$3(container, type$$1, function (event) {
          tapEvent.fireIfReady(event, type$$1).each(function (tapStopped) {
            if (tapStopped) {
              event.kill();
            }
          });
          var stopped = settings.triggerEvent(type$$1, event);
          if (stopped) {
            event.kill();
          }
        });
    ...
    var resume$1 = function (cWin, frame) {
      ...
      cWin.focus();
      focus$2(Element.fromDom(cWin.document.body));
